...
class SpeedConverterActivity : AppCompatActivity() {
    ...
    private var fromUnit = "m/s"
    private var toUnit = "m/s"
    ...
    private fun setupSpinners() {
        ...
        findViewById<Spinner>(R.id.length_array_from).also { spinner ->
            spinner.adapter = makeAdapter(R.array.speed_array_from)
        ...
        findViewById<Spinner>(R.id.length_array_to).also { spinner ->
            spinner.adapter = makeAdapter(R.array.speed_array_to)
            spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
    ...
    private fun convert() {
        ...
        val base = input * (toMetersPerSecond[fromUnit] ?: 1.0)
        val result = base / (toMetersPerSecond[toUnit] ?: 1.0)
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = getString(R.string.speed)
    }
