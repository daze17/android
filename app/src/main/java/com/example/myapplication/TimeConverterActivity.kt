...
class TimeConverterActivity : AppCompatActivity() {
    ...
    private var fromUnit = "Second"
    private var toUnit = "Second"
    ...
    private fun setupSpinners() {
        ...
        findViewById<Spinner>(R.id.length_array_from).also { spinner ->
            spinner.adapter = makeAdapter(R.array.time_array_from)
        ...
        findViewById<Spinner>(R.id.length_array_to).also { spinner ->
            spinner.adapter = makeAdapter(R.array.time_array_to)
            spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
    ...
    private fun convert() {
        ...
        val base = input * (toSeconds[fromUnit] ?: 1.0)
        val result = base / (toSeconds[toUnit] ?: 1.0)
