...
class DensityConverterActivity : AppCompatActivity() {

    private val toKgPerM3 = mapOf(
        "kg/m³" to 1.0,
        "g/cm³" to 1000.0,
        "lb/ft³" to 16.0185
    )

    private var fromUnit = "kg/m³"
    private var toUnit = "kg/m³"
    ...
    private fun setupSpinners() {
        ...
        findViewById<Spinner>(R.id.length_array_from).also { spinner ->
            spinner.adapter = makeAdapter(R.array.density_array_from)
        ...
        findViewById<Spinner>(R.id.length_array_to).also { spinner ->
            spinner.adapter = makeAdapter(R.array.density_array_to)
            spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
    ...
    private fun convert() {
        ...
        val base = input * (toKgPerM3[fromUnit] ?: 1.0)
        val result = base / (toKgPerM3[toUnit] ?: 1.0)
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = getString(R.string.density)
    }
