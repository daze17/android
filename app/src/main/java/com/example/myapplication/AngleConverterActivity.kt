...
class AngleConverterActivity : AppCompatActivity() {

    private val toDegrees = mapOf(
        "Degree" to 1.0,
        "Radian" to 57.2958,
        "Gradian" to 0.9
    )

    private var fromUnit = "Degree"
    private var toUnit = "Degree"
    ...
    private fun setupSpinners() {
        ...
        findViewById<Spinner>(R.id.length_array_from).also { spinner ->
            spinner.adapter = makeAdapter(R.array.angle_array_from)
        ...
        findViewById<Spinner>(R.id.length_array_to).also { spinner ->
            spinner.adapter = makeAdapter(R.array.angle_array_to)
            spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
    ...
    private fun convert() {
        ...
        val base = input * (toDegrees[fromUnit] ?: 1.0)
        val result = base / (toDegrees[toUnit] ?: 1.0)
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = getString(R.string.angle)
    }
